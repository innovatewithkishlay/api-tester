...
interface ApiFormProps {
  /** Callback so App can save history globally */
  onNewHistoryItem?: (item: HistoryItem) => void;
}

const ApiForm: React.FC<ApiFormProps> = ({ onNewHistoryItem }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Save history globally via App, or locally if not provided
      ...
      if (onNewHistoryItem) {
        ...
        const updated = [historyItem, ...parsed].slice(0, 20);
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-2xl font-semibold mb-2">API Request Builder</h2>
